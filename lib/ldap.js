...
function createClient () {
  var client = ldap.createClient({
    ...
    credentials:    nconf.get("LDAP_BIND_PASSWORD"),
    maxConnections: 5,
  ...
  client.bind(nconf.get("LDAP_BIND_USER"), nconf.get("LDAP_BIND_PASSWORD"), function(err) {
    if(err){
...
function createBinder () {
  ...
  binder.bind(nconf.get("LDAP_BIND_USER"), nconf.get("LDAP_BIND_PASSWORD"), function(err) {
    if(err){
      console.error("Error binding to LDAP", 'dn: ' + err.dn + '\n code: ' + err.code + '\n message: ' + err.message);
      return exit(1);
    }
