...
async function getPrimaryGroups(config, client, baseDn, user) {
  if (!user || !user.primaryGroupID || !user.objectSid) {
    console.log('Unable to retrieve the primary group info');
  ...
  try {
    primaryGroup = await search(client, baseDn, {
      scope: 'sub',
    ...
    console.log('Couldn\'t find the primary group', e);
  ...
  if (!primaryGroup || !primaryGroup.dn) {
    console.log('Failed to find the primary group for the user');
    return new Error('Failed to get the primary group or primary group is missing distinguishedName');
  }

  let parentGroups = null;
  try {
    parentGroups = await search(client, baseDn, {
      scope: 'sub',
      filter: config.get('LDAP_SEARCH_PRIMARY_PARENT_GROUPS').replace(/\{0\}/ig, primaryGroup.dn),
      attributes: _.uniq(['cn', 'dn', 'memberOf'].concat(config.get('GROUP_PROPERTIES'))),
      timeLimit: config.get('GROUPS_TIMEOUT_SECONDS'),
      derefAliases: config.get('GROUPS_DEREF_ALIASES')
    });
  } catch (e) {
    console.log('Couldn\'t find the primary groups', e);
  }
}
