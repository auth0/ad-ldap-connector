...
Users.prototype.list = function (search, options, callback) {
  ...
  if (search) {
    ...
    filterString = nconf.get('LDAP_SEARCH_QUERY').replace(/\{0\}/ig, searchQuery);
  } else {
    filterString = nconf.get('LDAP_SEARCH_ALL_QUERY');
  }
...
Users.prototype.listGroups = function(options, callback) {
  ...
  const filter = parseLdapFilter(nconf.get('LDAP_SEARCH_LIST_GROUPS_QUERY'));
  if (!filter.isValid) return callback(null, []);  // Search query is not valid, return empty results
...
Users.prototype._getAllGroupsAD = function (dn, callback) {
  ...
  const filter = parseLdapFilter(nconf.get('LDAP_SEARCH_GROUPS').replace(/\{0\}/ig, escapedDn));
  if (!filter.isValid) return callback(null, []);  // Search query is not valid, return empty results
