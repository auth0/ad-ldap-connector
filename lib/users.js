...
Users.prototype.getByUserName = function (userName, callback) {
  ...
  var opts = { scope: 'sub' };
  try {
    opts.filter = ldap.parseFilter(nconf.get('LDAP_USER_BY_NAME').replace(/\{0\}/g, userName));
  } catch (err) {
    return callback(null);  // Input username does not resolve to valid ldap filter, return no match
  }
...
Users.prototype.list = function (search, options, callback) {
  ...
  var opts = { scope: 'sub' };
  try {
    const filter = search ? nconf.get('LDAP_SEARCH_QUERY').replace(/\{0\}/ig, search) : nconf.get('LDAP_SEARCH_ALL_QUERY');
    opts.filter = ldap.parseFilter(filter);
  } catch (err) {
    return callback(null, []);  // Search query is not valid, return empty results
  }
...
Users.prototype.listGroups = function(options, callback) {
  ...
  try {
    opts.filter = ldap.parseFilter(nconf.get('LDAP_SEARCH_LIST_GROUPS_QUERY'));
  } catch (err) {
    return callback(null, []);  // Search query is not valid, return empty results
  }
...
Users.prototype._getAllGroupsAD = function (dn, callback) {
  ...
  try {
    opts.filter = ldap.parseFilter(nconf.get('LDAP_SEARCH_GROUPS').replace(/\{0\}/ig, dn));
  } catch (err) {
    return callback(null, []);  // Search query is not valid, return empty results
  }
