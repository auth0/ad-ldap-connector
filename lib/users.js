...
Users.prototype._getAllGroupsADCached = function (profile, callback) {
  ...
  async.map(memberOf, function (dn, done) {
    ...
    return self._getAllGroupsAD(profile, function (err, groups) {
      if (err) {
        return callback(err);
      }
...
Users.prototype._getAllGroupsAD = function(profile, callback) {
  var self = this;
  ...
  async.parallel([
    async function(cb) {
      try {
        ...
        let primaryGroups = await groups.getPrimaryGroups(nconf, self._client, self._baseGroups, profile);
