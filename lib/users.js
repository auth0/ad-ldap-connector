...
var PagedResultsControl = require('ldapjs').PagedResultsControl;
...
Users.prototype.listGroups = function(options, callback) {
  if (nconf.get('GROUPS_PAGING') === 'cache') {
    return this._listGroupsPagingFromCache(options, callback);
  }

  return this._listGroupsPagingManually(options, callback);
};

Users.prototype._listGroupsPagingFromCache = function(options, callback) {
  const self = this;
  const properties = nconf.get('GROUP_PROPERTIES').length ? nconf.get('GROUP_PROPERTIES') : [nconf.get('GROUP_PROPERTY')];
  ...
  const done = cb(function (err, result) {
    if (err) { return callback(err); }
    if (!result) {
      console.error('No result from the LDAP query. This may be due to an error.');
      return callback(new Error('Failed to list groups.'));
    }
    ...
    self._groupsCache.getPagedGroups(pageNumber, pageSize, callback);
  }).timeout(nconf.get('GROUPS_TIMEOUT_SECONDS') * 1000)
    .once();

  self._groupsCache.areAllGroupsCached(function (err, allGroupsCached) {
    if (err) { return done(callback); }
    if (allGroupsCached) {
      return self._groupsCache.getPagedGroups(pageNumber, pageSize, callback);
    }

    var opts = {
      scope: 'sub',
      filter: nconf.get('LDAP_SEARCH_LIST_GROUPS_QUERY'),
      // We need 'dn' for caching purposes
      attributes: properties.indexOf('dn') < 0 ? ['dn'].concat(properties) : properties,
      paged: {
        pageSize,
        pagePause: true
      }
    ...
    self._client.search(self._base, opts, function (err, res) {
      ...
      var ops = self._groupsCache.newInsertAllGroupsBatch();
      res.on('searchEntry', function (entry) {
        entry = getProperObject(entry);
        ops.addGroupOperation(entry);
      });
  
      res.on('error', done);
      res.on('page', function (result, nextPage) {
        if (!nextPage) { nextPage = result; }

        ops.flushCurrentOperations(function (err) {
          if (err) { return done(err); }
          nextPage();
        });
      });
      res.on('end', function (result) {
        ops.finishCurrentBatch(function (err) {
          if (err) { return done(err); }
          done(null, result);
        });
      });
...
Users.prototype._listGroupsPagingManually = function(options, callback) {
  var self = this;
  ...
  var pagingCookie = options.pagingCookie ? new Buffer(options.pagingCookie, 'binary') : '';
  var pagedResultsControl = new PagedResultsControl({
    type: PagedResultsControl.OID,
    value: {
      size: options.pageSize || nconf.get('GROUPS_PAGE_SIZE'),
      cookie: pagingCookie
    }
  });
  
  self._client.search(self._base, opts, pagedResultsControl, function (err, res) {
    if (err) {
      console.error('List groups error:', err);
      return callback(err);
    }
    ...
    var done = cb(function (result) {
      ...
      async.map(entries, function(entry, cb) {
        ...
        const pagingCookie = getPagingCookie(result.controls);
        const metadata = pagingCookie ? { pagingCookie } : {};
...
Users.prototype._getAllGroupsADCached = function (profile, callback) {
  ...
  async.map(memberOf, self._groupsCache.getGroup, function (err, fromCache) {
...
function getPagingCookie(controls) {
  controls = controls || [];
  const pagingControl = controls.find(c => c instanceof PagedResultsControl);
  return pagingControl && pagingControl.value && pagingControl.value.cookie && pagingControl.value.cookie.toString('binary');
}