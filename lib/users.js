...
Users.prototype.getByUserName = function (userName, callback) {
  ...
  const filter = parseLdapFilter(nconf.get('LDAP_USER_BY_NAME').replace(/\{0\}/g, userName));
...
Users.prototype.list = function (search, options, callback) {
  ...
  const filterString = search ? nconf.get('LDAP_SEARCH_QUERY').replace(/\{0\}/ig, search) : nconf.get('LDAP_SEARCH_ALL_QUERY');
