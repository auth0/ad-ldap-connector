...
const escapeLdapFilter = require('./escapeLdapFilter');
const parseLdapFilter = require('./parseLdapFilter');
...
var Users = module.exports = function (disable_caching) {
  ...
  this._base = config.get('LDAP_BASE');
  this._baseGroups = config.get('LDAP_BASE_GROUPS') || config.get('LDAP_BASE');
  ...
  }, parseInt(config.get('LDAP_NUMBER_OF_PARALLEL_BINDS'), 10));
 ...
 */
Users.prototype.validate = function (userName, password, options, callback) {
  var self = this;
  ...
  self.getByUserName(userName, options, function (err, profile) {
    if (err) {
 ...
 *
 */
Users.prototype.changePassword = function (userName, password, options, callback) {
  var self = this;
  var log = logger(userName);
  ...
  self.getByUserName(userName, options, function (err, profile) {
    ...
    var modification = {};
    if(config.get('ENABLE_ACTIVE_DIRECTORY_UNICODE_PASSWORD') === true){
    ...
    if (config.get('AUTO_UNLOCK_ON_PASSWORD_CHANGE') === true) {
...
Users.prototype.getByUserName = function (userName, options, callback) {
  var self = this;
  ...
  const searchUsername = options.escaped ? userName : escapeLdapFilter(userName);
  const filter = parseLdapFilter(config.get('LDAP_USER_BY_NAME').replace(/\{0\}/g, searchUsername));
...
Users.prototype.list = function (search, options, callback) {
  ...
  var filterString;
  if (search) {
    const searchQuery = options.escaped ? search : escapeLdapFilter(search);
    filterString = config.get('LDAP_SEARCH_QUERY').replace(/\{0\}/ig, searchQuery);
  } else {
    filterString = config.get('LDAP_SEARCH_ALL_QUERY');
  }

  const filter = parseLdapFilter(filterString);
  if (!filter.isValid) return callback(null, []);  // Search query is not valid, return empty results
  ...
  self._client.search(self._base, opts, function (err, res) {
    ...
    function done() {
      ...
      async.map(entries, function (e, done) {
        self.enrichProfile(e, config.get('LDAP_SEARCH_RESULTS_OMIT_GROUPS'), done);
      }, callback);
    }
...
Users.prototype.listGroups = function(options, callback) {
  ...
  const filter = parseLdapFilter(config.get('LDAP_SEARCH_LIST_GROUPS_QUERY'));
  if (!filter.isValid) return callback(null, []);  // Search query is not valid, return empty results

  var opts = {
    scope: 'sub',
    filter: filter.filter,
    attributes: config.get('GROUP_PROPERTIES').length > 0 ? config.get('GROUP_PROPERTIES') : [config.get('GROUP_PROPERTY')]
  };
  if (options.limit) opts.sizeLimit = parseInt(options.limit, 10);
  ...
  self._client.search(self._base, opts, function (err, res) {
    ...
    var done = cb(function (result) {
      ...
      async.map(entries, function(entry, cb) {
        if (config.get('GROUP_PROPERTIES').length > 0) {
          return cb(null, _.pick(entry, config.get('GROUP_PROPERTIES')));
        }
        return cb(null, entry[config.get('GROUP_PROPERTY')]);
...
Users.prototype.enrichProfile = function (profile, noGroups, callback) {
  ...
  async.parallel([
    function (done) {
      if (noGroups || config.get('GROUPS') === false) {
      ...
      self._getAllGroupsADCached(profile, function (err, groups) {
        ...
        var result = groups.map(function (group) {
          if (config.get('GROUP_PROPERTIES').length > 0) {
            return _.pick(group, config.get('GROUP_PROPERTIES'));
          }
          return group[config.get('GROUP_PROPERTY')];
        });
        done(null, result);
      });
    ...
    profileMapper.mapProfile(profile)
      .then(mappedProfile => callback(null, mappedProfile))
      .catch(err => callback(err));
  });
};

Users.prototype._getAllGroupsADCached = function (profile, callback) {
  var memberOf = Array.isArray(profile.memberOf) ? profile.memberOf : profile.memberOf ? [profile.memberOf] : [];
  ...
  const resolveFromCache = fromCache.length > 0 && fromCache.every(Boolean);
...
Users.prototype._getAllGroupsAD = function (dn, callback) {
  ...
  const filter = parseLdapFilter(config.get('LDAP_SEARCH_GROUPS').replace(/\{0\}/ig, escapedDn));
  if (!filter.isValid) return callback(null, []);  // Search query is not valid, return empty results
  ...
  callback = cb(callback)
    .timeout(config.get('GROUPS_TIMEOUT_SECONDS') * 1000)
    .once();
