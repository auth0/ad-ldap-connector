 ...
 */
function readPageElements(entries, pageNumber, pageSize) {
  var current = 1;
...
/**
 * Reads 'pageSize' number of items of the specified 'pageNumber' from the Cache
 * 
 * @param {number} pageNumber 
 * @param {number} pageSize 
 * @param {function} callback 
 */
Users.prototype.readPageFromCache = function(pageNumber, pageSize, callback) {
  const entries = [];
  this._groupsCache.createReadStream({ gte: 'group|', lte: 'group|~', })
    .on('data', readPageElements(entries, pageNumber, pageSize))
    .on('error', callback)
    .on('end', function () {
...
function checkGroupsCachingStatus(_groupsCache, callback) {
  _groupsCache.get(ALL_GROUPS_KEY, function (err) {
    if (err) {
      if (err.notFound) {
...
Users.prototype.listGroups = function(options, callback) {
  ...
  const done = cb(function (err, result) {
    ...
    self.readPageFromCache(pageNumber, pageSize, callback);
    ...
    self._client.search(self._base, opts, function (err, res) {
      ...
      res.on('searchEntry', function (entry) {
        ...
        ops.push({ type: 'put', key: `group|${entry.dn}`, value: JSON.stringify(entry) });
      });
      ...
      res.on('page', function (result, nextPage) {
        ...
        self._groupsCache.batch(ops, CACHE_TTL_OPTIONS, function (err) {
      ...
      res.on('end', function (result) {
        ops.push({ type: 'put', key: ALL_GROUPS_KEY, value: 'OK' });
...
Users.prototype._getAllGroupsADCached = function (profile, callback) {
  ...
  async.map(memberOf, function (dn, done) {
    self._groupsCache.get(`group|${dn}`, function (err, groups) {
    ...
    return self._getAllGroupsAD(profile.dn, function (err, groups) {
      ...
      graph.flatDeps(groups).forEach(function (fg) {
        self._groupsCache.put(`group|${fg.dn}`, JSON.stringify(fg), CACHE_TTL_OPTIONS);
      });
