...
Users.prototype._getAllGroupsADCached = function (profile, callback) {
  ...
  async.map(memberOf, function (dn, done) {
    ...
    return self._getAllGroupsAD(profile.dn, profile.primaryGroupID, function (err, groups) {
      if (err) {
        return callback(err);
      }
...
Users.prototype._getAllGroupsAD = function(dn, primaryGroupId, callback) {
  var self = this;
    ...
    async function(cb) {
      try {
        ...
        let primaryGroups = await findGroups(self._client, self._baseGroups, {
          scope: 'sub',
          filter: nconf.get('LDAP_SEARCH_PRIMARY_GROUP').replace(/\{0\}/ig, dn),
  ...
  function(err, results) {
    if (err) {
      ...
      return callback(null, []); // This mimics previous behavior of ignoring errors
    }
